...
class Agent(object):
    def __init__(self, gamma, epsilon, lr, n_actions, input_dims,  mem_size, batch_size, eps_min=0.01, eps_dec=5e-7,
        ...
        # 经过replace_target_counter轮次将训练的network替换掉target
        self.replace_target_counter = replace
        self.algo = algo
        self.env_name = env_name
        self.checkpoint_dir = checkpoint_dir
        # 按照动作数量 创建一个列表 表示所有动作 假如n = 4 则space[0,1,2,3]
    ...
    def learn(self):
        ...
        # np.arange[]递增返回索引 如果size = 4 indexes = [0,1,2,3]
        indexes = np.arange(self.batch_size)

        # 单纯的Duel 没有用到DDQN和Prior
